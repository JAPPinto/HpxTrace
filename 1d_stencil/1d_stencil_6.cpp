...
{
    //std::cout << "i:" << i << " np:" << np << " nl:" << nl << " loc:" << (i * nl) / np << std::endl;
    return (i * nl) / np;
}
/*
3 localidades 6  nps
  ...
  : hpx::components::component_base<partition_server>
{
    ...
    partition_server(partition_data const& data, int partition_id)
      : data_(data), partition_id(partition_id)
    {}

    partition_server(std::size_t size, double initial_value,int partition_id)
      : data_(size, initial_value), partition_id(partition_id)
    {}
    ...
    HPX_DEFINE_COMPONENT_DIRECT_ACTION(partition_server, get_partition_id, get_partition_id_action);
...
typedef partition_server::get_partition_id_action get_partition_id_action;
HPX_REGISTER_ACTION(get_partition_id_action);
...
struct partition : hpx::components::client_base<partition, partition_server>
{
    ...
    partition(hpx::id_type where, std::size_t size, double initial_value, int partition_id)
      : base_type(hpx::new_<partition_server>(where, size, initial_value, partition_id))
    {}
    ...
    partition(hpx::id_type where, partition_data const& data, int partition_id)
      : base_type(hpx::new_<partition_server>(hpx::colocated(where), data, partition_id))
    {}
    ...
    int get_partition_id() const
    {
        partition_server::get_partition_id_action act;
        return act(get_id());
    }
};
...
struct stepper
{
        ...
        partition const& right)
    {
        ...
        HpxTrace::trigger_probe("start_heat", {{"&partition_id", middle.get_partition_id()}}, {{"s", "ola"}} );
        //std::cout << "START heat_part " << reinterpret_cast<std::uint64_t>(hpx::threads::get_self_id().get()) << std::endl;
        //std::cout << "AAAAAAAAAAAA " << hpx::thread_instance::instance().get_id() << std::endl;
        //std::cout << "PART " << middle.get_partition_id() << std::endl;
        ...
        return dataflow(
            ...
            unwrapping(
                ...
                {
                    HpxTrace::trigger_probe("received_data", {{"&partition_id", middle.get_partition_id()}}, {{"s", "ola"}} );
                    ...
                    // as 'middle'.
        //std::cout << "END heat_part " << reinterpret_cast<std::uint64_t>(hpx::threads::get_self_id().get()) << std::endl;
                    partition p = partition(middle.get_id(), heat_part_data(l, m, r), middle.get_partition_id());
            HpxTrace::trigger_probe("end_heat", {{"&partition_id", middle.get_partition_id()}}, {{"s", "ola"}} );
                    return p;
                }
            ), "heat_part_data"),
...
stepper::space stepper::do_work(std::size_t np, std::size_t nx, std::size_t nt)
{
    ...
    for (std::size_t i = 0; i != np; ++i)
        U[0][i] = partition(localities[locidx(i, np, nl)], nx, double(i), i);
    //]
...
///////////////////////////////////////////////////////////////////////////////
int hpx_main1(hpx::program_options::variables_map& vm)
{
    ...
    // U[t][i] is the state of position i at time t.
    std::vector<stepper::space> U(2);
    for (stepper::space& s: U)
        s.resize(np);

    // Initial conditions: f(0, i) = i
    //[do_work_6
    //for (std::size_t i = 0; i != 1; ++i)
    //    partition(localities[1], nx, double(i));
        //U[0][i] = 
    //]
   // hpx::new_<partition_server>(localities[0]);//, size, initial_value)
    //hpx::new_<partition_server>(localities[0]);//, size, initial_value)
        partition p = partition(localities[1], nx, double(1),1);
        //p.get_data(partition_server::left_partition);
        ...
        partition_server::get_data_action act;
        usleep(100000);
        //hpx::async(act, p.get_id(), partition_server::left_partition);
        //hpx::apply(act, p.get_id(), partition_server::left_partition);
        act(p.get_id(), partition_server::left_partition);
