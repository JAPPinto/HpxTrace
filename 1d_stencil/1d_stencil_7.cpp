...
struct stepper
{
        ...
        partition const& middle, partition const& right)
    {
        ...
        HpxTrace::trigger_probe("start_heat", {{"a",0}}, {{"s", "ola"}} );
        ...
        hpx::future<partition_data> next_middle = middle_data.then(
            ...
            unwrapping(
                [middle](partition_data const& m) -> partition_data
                {
                    ...
                    HpxTrace::trigger_probe("finished_middle", {{"a",0}}, {{"s", "ola"}} );
                    return next;
                }
            ),"middle_heat_part")
        );
        ...
        return dataflow(
            ...
            unwrapping(
                    ...
                    partition_data const& m, partition_data const& r) -> partition
                {
                    ...
                    HpxTrace::trigger_probe("received_data", {{"a",0}}, {{"s", "ola"}} );
                    ...
                    // as 'middle'.
                            HpxTrace::trigger_probe("end_heat", {{"a",0}}, {{"s", "ola"}} );
...
// time steps
/*
stepper::space stepper::do_work2(std::size_t np, std::size_t nx, std::size_t nt)
{
    using hpx::dataflow;
