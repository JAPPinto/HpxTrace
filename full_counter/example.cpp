...
namespace performance_counters { namespace example
{
    ...
    hpx::program_options::options_description command_line_options()
    {
        hpx::program_options::options_description opts(
            "Additional command line options for the example component");
        opts.add_options()
            ("example", "enables the performance counters implemented by the "
                "example component")
            ;
        return opts;
    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    // Parse the command line to figure out whether the example performance
    ...
    bool need_perf_counters()
    {
        ...
        if (!retrieve_commandline_arguments(command_line_options(), vm))
        {
            HPX_THROW_EXCEPTION(hpx::commandline_option_error,
                "example::need_perf_counters",
                "Failed to handle command line options");
            return false;
        }

        // We enable the performance counters if --example is specified on the
        // command line.
        return vm.count("example") != 0;
    }
    ...
    void startup()
    {
        ...
        {
            { "/example/immediate/explicit", counter_raw,
              "returns the current value of a example wave calculated over "
              "an arbitrary time line (explicit, hand-rolled version)",
              HPX_PERFORMANCE_COUNTER_V1,
              // We assume that valid counter names have the following scheme:
              //
              //  /example(locality#<locality_id>/instance#<instance_id>)/immediate/explicit
              //
            ...
            { "/example/immediate/implicit", counter_raw,
              "returns the current value of a example wave calculated over "
              "an arbitrary time line (implicit version, using HPX facilities)",
              HPX_PERFORMANCE_COUNTER_V1,
              // We assume that valid counter names have the following scheme:
              //
              //  /example(locality#<locality_id>/total)/immediate/implicit
              //
              // where '<locality_id>' is the number of the locality the
              // counter has to be instantiated on. The function 'immediate_example'
    ...
    {
        // exit silently if this gets loaded outside of the example_client example
        if (hpx::get_config_entry("hpx.components.example.enabled", "0") == "0")
        {
            return false;
        }
        ...
        if (!need_perf_counters())
        {
            HPX_THROW_EXCEPTION(hpx::dynamic_link_failure,
                "performance_counters::example::get_startup",
                "the example component is not enabled on the commandline "
                "(--example), bailing out");
            return false;
        }
...
HPX_REGISTER_COMMANDLINE_MODULE_DYNAMIC(
    ::performance_counters::example::command_line_options);
