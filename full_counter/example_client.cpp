...
// performance counters. It utilizes the example component, which implements two
...
//
// /example{locality#0/instance#0}/immediate/explicit
//    This is a custom performance counter fully implemented in the example
//    component. It evaluates a new value every 1000ms (1s) and delivers this
//    value whenever it is queried.
//
// /example{locality#0/total}/immediate/implicit
...
//    value is queried. It calculates the current value of a example based on the
//    uptime of the counter.
//
// /statistics{/example{locality#0/instance#1}/immediate/explicit}/average#100
//    This is an aggregating counter calculating the average value of a given
//    base counter (in this case /example{locality#0/instance#1}/immediate/explicit,
...
//    counter name). No special code in the example example is needed for this
...
int monitor(std::uint64_t pause, std::uint64_t values)
{
    ...
    performance_counter example_explicit(hpx::util::format(
        "/example{{locality#{}/instance#{}}}/immediate/explicit",
        prefix, 0));
    performance_counter example_implicit(hpx::util::format(
        "/example{{locality#{}/total}}/immediate/implicit",
        prefix));
    performance_counter example_average(hpx::util::format(
        "/statistics{{/example{{locality#{}/instance#{}}}/immediate/explicit}}/average@100",
        prefix, 1));
    ...
    // 'false'.
    example_explicit.start();
    example_implicit.start();
    example_average.start();
    ...
    while (values-- > 0)
    {
        ...
        counter_value value1 = example_explicit.get_counter_value(hpx::launch::sync);
        counter_value value2 = example_implicit.get_counter_value(hpx::launch::sync);
        counter_value value3 = example_average.get_counter_value(hpx::launch::sync);
        ...
        // stop/restart the example_explicit counter after every 5 seconds of
        // evaluation
        bool should_run =
...
int hpx_main(hpx::program_options::variables_map& vm)
{
    ...
    std::cout << "starting example monitoring..." << std::endl;
    ...
    try {
        ...
        std::cerr << "example_client: caught exception: " << e.what() << std::endl;
        std::cerr << "Have you specified the command line option "
                     "--example to enable the example component?"
                  << std::endl;
    }
...
int main(int argc, char* argv[])
{
    ...
    options_description desc_commandline("usage: example_client [options]");
    ...
    std::vector<std::string> cfg = {
        "hpx.components.example.enabled! = 1"
    };
